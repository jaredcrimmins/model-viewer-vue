...
import {deserializeUrl, timePasses} from '../utils';
...
export default class TextureUtils extends EventDispatcher {
  private dynamicEnvironmentMap: Promise<CubeTexture> | null = null;
  ...
  async load(
    ...
    try {
      ...
      if (!isHDR) texture.encoding = sRGBEncoding;
  ...
  /**
   * Returns a Texture from a WebGLRenderCubeTarget.
   * */
  async generateEnvironmentMap(
    environmentMapUrl: string | null = null,
    options: EnvironmentGenerationConfig = {}
  ): Promise<Texture> {
    const {progressTracker} = options;
    const useDynamicEnvironment = environmentMapUrl === 'neutral';

    if (useDynamicEnvironment) environmentMapUrl = null;
    ...
    if (environmentMapUrl) {
      ...
      environmentMapLoads = this.loadDynamicEnvironmentMap();
    }
   ...
   * object.
   */
  private async loadDynamicEnvironmentMap(): Promise<CubeTexture> {
    if (this.dynamicEnvironmentMap === null) {
      this.dynamicEnvironmentMap =
        this.generateDynamicEnvironmentMap(new EnvironmentScene(), 'neutral');
    }

    return this.dynamicEnvironmentMap;
  }

  private async generateDynamicEnvironmentMap(scene: Scene, name: string) {
    await timePasses();
  ...
  private halfblur(
    ...
    sigmaRadians: number,
    direction: 'latitudinal' | 'longitudinal'
  ) {
  ...
  async dispose() {
    ...
    if (this.dynamicEnvironmentMap !== null) {
      (await this.dynamicEnvironmentMap).dispose();
      this.dynamicEnvironmentMap = null;
    }
